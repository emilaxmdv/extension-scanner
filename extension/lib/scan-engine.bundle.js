/* MalXtension — Scan Engine v3.0.0 | Bundled for browser */
...
var ScanEngine = (() => {
  ...
  // scan-engine/src/index.js
  var src_exports = {};
  __export(src_exports, {
    BEHAVIORAL_WEIGHTS: () => BEHAVIORAL_WEIGHTS,
    MAX_SCORE: () => MAX_SCORE,
    PERMISSION_WEIGHTS: () => PERMISSION_WEIGHTS,
    RISK_LEVELS: () => RISK_LEVELS,
    analyzeExtension: () => analyzeExtension,
    calculateCapabilityScore: () => calculateCapabilityScore,
    calculateExposureMultiplier: () => calculateExposureMultiplier,
    detectBehavioralIndicators: () => detectBehavioralIndicators,
    detectObfuscation: () => detectObfuscation,
    evaluateEscalationRules: () => evaluateEscalationRules,
    exportResults: () => exportResults,
    extractHostPermissions: () => extractHostPermissions,
    getExposureDescription: () => getExposureDescription,
    getPermissionDescriptions: () => getPermissionDescriptions,
    isExposureLimited: () => isExposureLimited,
    quickScan: () => quickScan
  });

  // scan-engine/src/constants.js
  var PERMISSION_WEIGHTS = {
    // Critical (9-10)
    debugger: 10,
    nativeMessaging: 9,
  ...
  var EXPOSURE_MULTIPLIERS = {
    NO_HOSTS: 1,
    SINGLE_DOMAIN: 1,
    MULTIPLE_DOMAINS: 1.5,
    WILDCARD_SUBDOMAIN: 2,
    ALL_URLS: 3,
    FILE_ACCESS: 1.8
  };
  var BEHAVIORAL_WEIGHTS = {
  ...
  // scan-engine/src/utils/helpers.js
  ...
  function extractBase64Strings(code) {
    ...
    while ((m = re.exec(code)) !== null) {
      if (isBase64(m[1]))
        results.push({ value: m[1], length: m[1].length });
    }
    return results;
  }
  function analyzeVariableNames(identifiers) {
  ...
  function calculateCodeDensity(code) {
    if (!code)
      return { totalChars: 0, totalLines: 0, charsPerLine: 0, nonEmptyLines: 0 };
  ...
  function categorizeHostPermissions(hosts) {
    if (!hosts || hosts.length === 0)
      return "NO_HOSTS";
    const allUrl = hosts.some(
      (h) => h === "<all_urls>" || h === "*://*/*" || h === "http://*/*" || h === "https://*/*"
    );
    if (allUrl)
      return "ALL_URLS";
    if (hosts.some((h) => h.startsWith("file://")))
      return "FILE_ACCESS";
    if (hosts.some((h) => h.includes("*") && h !== "<all_urls>"))
  ...
  function getRiskLevel(score, levels) {
  ...
  // scan-engine/src/analyzers/capability-scorer.js
  function calculateCapabilityScore(manifest) {
    if (!manifest)
      return { score: 0, permissions: [], highRiskPermissions: [], details: {} };
    ...
    return {
      ...
      details,
      highRiskPermissions: Object.entries(details).filter(([, w]) => w >= 6).map(([p]) => p)
    };
  }
  function extractHostPermissions(manifest) {
    if (!manifest)
      return [];
    const hosts = /* @__PURE__ */ new Set();
    const add = (arr) => {
      if (Array.isArray(arr))
        arr.forEach((h) => hosts.add(h));
    };
  ...
  // scan-engine/src/analyzers/exposure-calculator.js
  function calculateExposureMultiplier(hostPermissions) {
    ...
    return { multiplier: EXPOSURE_MULTIPLIERS[category] || 1, category, analysis };
  }
  ...
  function calculateExposureScore(capScore, multiplier) {
    return capScore * multiplier;
  }
  function isExposureLimited(category) {
    return ["NO_HOSTS", "SINGLE_DOMAIN", "MULTIPLE_DOMAINS"].includes(category);
  }
  function getExposureDescription(cat) {
    const m = {
  ...
  // scan-engine/src/detectors/behavioral-detector.js
  function detectBehavioralIndicators(jsFiles) {
  ...
  function analyzeFile(code, filename) {
    const I = {};
    for (const key of Object.keys(BEHAVIORAL_WEIGHTS))
      I[key] = 0;
    ...
    I.WEBGL_FINGERPRINT = rx(/getParameter\s*\(\s*(debugInfo|renderer|vendor)/g) + rx(/WEBGL_debug_renderer_info/g);
    ...
    I.CRYPTO_MINING_PATTERN = rx(/CoinHive|coinhive|cryptonight|stratum\+tcp/gi) + rx(/hashRate|nonce.*increment|mining.*pool/gi);
    ...
    try {
      const ast = parse3(code, { ecmaVersion: "latest", sourceType: "module", locations: false });
      simple(ast, {
        NewExpression(node) {
          const name = node.callee.name;
          if (name === "Function")
            I.FUNCTION_CONSTRUCTOR++;
          if (name === "WebSocket")
            I.WEBSOCKET_CONNECTION++;
          if (name === "XMLHttpRequest")
            I.XMLHTTPREQUEST_EXTERNAL++;
        },
        ImportExpression() {
          I.DYNAMIC_IMPORT++;
        },
        CallExpression(node) {
          ...
          if (node.callee.type === "MemberExpression" && node.callee.property.name === "createElement" && node.arguments.length > 0 && node.arguments[0].type === "Literal") {
          ...
          if (node.callee.type === "MemberExpression" && node.callee.property.name === "submit") {
  ...
  function buildSummary(indicators) {
    ...
    if (crit.length)
      s.push(`${crit.length} critical behavioural pattern(s) detected`);
    if (high.length)
      s.push(`${high.length} high-risk behavioural pattern(s) detected`);
    const find = (t) => indicators.find((i) => i.type === t);
    const ev = find("EVAL_USAGE");
    if (ev)
      s.push(`eval() used ${ev.count} time(s)`);
    const ext = find("EXTERNAL_FETCH");
    if (ext)
      s.push(`${ext.count} external network request(s)`);
    const fp = indicators.filter((i) => ["CANVAS_FINGERPRINT", "WEBGL_FINGERPRINT", "AUDIO_FINGERPRINT", "FONT_ENUMERATION"].includes(i.type));
    if (fp.length)
      s.push(`${fp.length} browser-fingerprinting technique(s)`);
    const mine = find("CRYPTO_MINING_PATTERN");
    if (mine)
      s.push("Possible crypto-mining code detected");
    return s;
  }

  // scan-engine/src/detectors/obfuscation-detector.js
  function detectObfuscation(jsFiles) {
    if (!jsFiles || jsFiles.length === 0) {
    ...
    for (const f of jsFiles) {
      const r = analyzeFile2(f.content, f.name);
      total += r.score;
    ...
    return {
      ...
      indicators,
      details: fileDetails,
      summary: buildSummary2(indicators)
    };
  }
  function analyzeFile2(code, filename) {
    const indicators = [];
    let score = 0;
    const ent = calculateEntropy(code);
    if (ent >= OBFUSCATION_THRESHOLDS.VERY_HIGH_ENTROPY) {
      score += OBFUSCATION_SCORES.VERY_HIGH_ENTROPY;
      indicators.push({ type: "ENTROPY", score: OBFUSCATION_SCORES.VERY_HIGH_ENTROPY, severity: "critical", entropy: ent, description: `Very high entropy ${ent.toFixed(2)} bits/char` });
    } else if (ent >= OBFUSCATION_THRESHOLDS.HIGH_ENTROPY) {
      score += OBFUSCATION_SCORES.HIGH_ENTROPY;
      indicators.push({ type: "ENTROPY", score: OBFUSCATION_SCORES.HIGH_ENTROPY, severity: "high", entropy: ent, description: `High entropy ${ent.toFixed(2)} bits/char` });
    }
    const ids = extractIdentifiers(code);
    const va = analyzeVariableNames(ids);
    if (va.veryShortRatio >= OBFUSCATION_THRESHOLDS.VERY_SHORT_VAR_RATIO) {
      score += OBFUSCATION_SCORES.VERY_SHORT_VARIABLES;
      indicators.push({ type: "VARIABLES", score: OBFUSCATION_SCORES.VERY_SHORT_VARIABLES, severity: "high", shortRatio: va.shortRatio, description: `${Math.round(va.shortRatio * 100)}% short variable names` });
    } else if (va.shortRatio >= OBFUSCATION_THRESHOLDS.SHORT_VAR_RATIO) {
      score += OBFUSCATION_SCORES.SHORT_VARIABLES;
      indicators.push({ type: "VARIABLES", score: OBFUSCATION_SCORES.SHORT_VARIABLES, severity: "medium", shortRatio: va.shortRatio, description: `${Math.round(va.shortRatio * 100)}% short variable names` });
    }
    const b64 = extractBase64Strings(code);
    const large = b64.filter((s) => s.length >= OBFUSCATION_THRESHOLDS.VERY_LARGE_BASE64_SIZE);
    const medium = b64.filter((s) => s.length >= OBFUSCATION_THRESHOLDS.LARGE_BASE64_SIZE && s.length < OBFUSCATION_THRESHOLDS.VERY_LARGE_BASE64_SIZE);
    if (large.length > 0) {
      score += OBFUSCATION_SCORES.VERY_LARGE_BASE64;
      indicators.push({ type: "BASE64", score: OBFUSCATION_SCORES.VERY_LARGE_BASE64, severity: "high", count: b64.length, largeCount: large.length, description: `${large.length} very large base64 string(s)` });
    } else if (medium.length > 0) {
      score += OBFUSCATION_SCORES.LARGE_BASE64;
      indicators.push({ type: "BASE64", score: OBFUSCATION_SCORES.LARGE_BASE64, severity: "medium", count: b64.length, description: `${medium.length} large base64 string(s)` });
    }
    const d = calculateCodeDensity(code);
    if (d.charsPerLine >= OBFUSCATION_THRESHOLDS.MIN_CHAR_TO_NEWLINE_RATIO * 2) {
      score += OBFUSCATION_SCORES.HEAVY_MINIFICATION;
      indicators.push({ type: "MINIFICATION", score: OBFUSCATION_SCORES.HEAVY_MINIFICATION, severity: "medium", charsPerLine: Math.round(d.charsPerLine), description: `Heavily minified (${Math.round(d.charsPerLine)} chars/line)` });
    } else if (d.charsPerLine >= OBFUSCATION_THRESHOLDS.MIN_CHAR_TO_NEWLINE_RATIO) {
      score += OBFUSCATION_SCORES.MINIFIED_CODE;
      indicators.push({ type: "MINIFICATION", score: OBFUSCATION_SCORES.MINIFIED_CODE, severity: "low", charsPerLine: Math.round(d.charsPerLine), description: `Minified code (${Math.round(d.charsPerLine)} chars/line)` });
    }
  ...
  function buildSummary2(indicators) {
    const s = [];
    const c = indicators.filter((i) => i.severity === "critical").length;
    const h = indicators.filter((i) => i.severity === "high").length;
    if (c)
      s.push(`Critical obfuscation: ${c} indicator(s)`);
    if (h)
      s.push(`High obfuscation: ${h} indicator(s)`);
    return s;
  }

  // scan-engine/src/analyzers/combination-rules.js
  function evaluateEscalationRules({ permissions = [], exposureCategory = "NO_HOSTS", behavioralIndicators = [], obfuscationScore = 0 }) {
    const triggered = [];
    let total = 0;
  ...
  function evaluateRule(rule, ctx) {
    ...
    if (conditions.permissions) {
      ...
      if (missing.length === 0)
        matched.push({ type: "permissions", details: conditions.permissions });
      else
        allMet = false;
    }
    if (conditions.exposure) {
      if (conditions.exposure.includes(ctx.exposureCategory))
        matched.push({ type: "exposure", details: ctx.exposureCategory });
      else
        allMet = false;
    }
    if (conditions.exposureLimited !== void 0) {
      const limited = isExposureLimited(ctx.exposureCategory);
      if (conditions.exposureLimited === limited)
        matched.push({ type: "exposureLimited", details: limited });
      else
        allMet = false;
    }
    if (conditions.behaviors) {
      const types2 = new Set(ctx.behavioralIndicators.map((b) => b.type));
      const found = conditions.behaviors.filter((b) => types2.has(b));
      if (found.length > 0)
        matched.push({ type: "behaviors", details: found });
      else
        allMet = false;
    }
    if (conditions.obfuscation !== void 0) {
      const obf = ctx.obfuscationScore >= 20;
      if (conditions.obfuscation === obf)
        matched.push({ type: "obfuscation", details: ctx.obfuscationScore });
      else
        allMet = false;
    }
  ...
  // scan-engine/src/analyzer.js
  async function analyzeExtension(manifest, jsFiles = []) {
    if (!manifest)
      throw new Error("manifest is required");
    if (!Array.isArray(jsFiles))
      throw new Error("jsFiles must be an array");
    ...
    return {
      ...
      summary: buildSummary3({ score, level, capResult, expResult, behResult, obfResult, escResult }),
      metadata: {
  ...
  async function quickScan(manifest, jsFiles = []) {
  ...
  function exportResults(results, includeDetails = true) {
    ...
    if (includeDetails)
      out.details = results.details;
    return JSON.stringify(out, null, 2);
  }
  function buildSummary3({ score, level, capResult, expResult, behResult, obfResult, escResult }) {
    ...
    if (expResult.category === "ALL_URLS")
      s.concerns.push("Extension has access to ALL websites");
    else if (expResult.category === "WILDCARD_SUBDOMAIN")
      s.concerns.push("Uses wildcard host permissions");
    if (behResult.summary.length)
      s.concerns.push(...behResult.summary);
    if (obfResult.isObfuscated)
      s.concerns.push("Code appears obfuscated");
    if (escResult.ruleCount > 0)
      s.concerns.push(`${escResult.ruleCount} dangerous permission combination(s)`);
...
//# sourceMappingURL=scan-engine.bundle.js.map
